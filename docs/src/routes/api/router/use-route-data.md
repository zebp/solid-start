...
To access the `:id` part of the route, call `useRouteData()` inside a component. The returned object `params` will have a field `id` that will match the `:id` part of the URL. For example, if the URL is `/users/123`, then `params.id` will be `123`.
...
function User() {
  const params = useRouteData<{ id: string }>();
...
// @errors: 2571
// @lib: ES2015
import { Router, Route, useRouteData } from "solid-app-router";
...
function User() {
  const params = useRouteData<{ id: string; project: string }>();
...
In some cases, you might want to create async resources within your component tree, outside the `routeData` function. Here, you would need to get the params from the `useRouteData` hook.

For example, if you have a route like `/users/:id`, then you can access the `id` param by using `useRouteData` inside your component.

```tsx twoslash {2}
// @errors: 2571
// @lib: ES2015
import { useRouteData, Router, Route } from "solid-app-router";
...
Call `useRouteData()` inside a component to get the current route params.
