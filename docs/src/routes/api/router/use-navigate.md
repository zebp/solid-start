...
To access the `:id` part of the route, call `useNavigate()` inside a component. The returned object `params` will have a field `id` that will match the `:id` part of the URL. For example, if the URL is `/users/123`, then `params.id` will be `123`.
...
function User() {
  const params = useNavigate<{ id: string }>();
...
// @errors: 2571
// @lib: ES2015
import { Router, Route, useNavigate } from "solid-app-router";
...
function User() {
  const params = useNavigate<{ id: string; project: string }>();
...
In some cases, you might want to create async resources within your component tree, outside the `routeData` function. Here, you would need to get the params from the `useNavigate` hook.

For example, if you have a route like `/users/:id`, then you can access the `id` param by using `useNavigate` inside your component.

```tsx twoslash {2}
// @errors: 2571
// @lib: ES2015
import { useNavigate, Router, Route } from "solid-app-router";
...
Call `useNavigate()` inside a component to get the current route params.
